...
#include "dcbuffer/LayoutCodex.h"
#include <algorithm>
#include <cctype>
#include <string>
...
namespace gfx::dcb
{
	...
	std::string
	LayoutElement::GetSignature() const
	{
		switch (type)
		{
#define X(el) \
...
#undef X
	...
	bool
	LayoutElement::Exists() const noexcept
	{
		return type != Empty;
	}
	std::pair<size_t, const LayoutElement*>
	LayoutElement::CalculateIndexingOffset(size_t a_offset, size_t index) const
	{
		assert("Indexing into non-array" && type == Array);
		const auto& data = static_cast<ExtraData::Array&>(*pExtraData);
		assert(index < data.size);
		return { a_offset + data.layoutElement->GetSizeInBytes() * index, &*data.layoutElement };
	}
	LayoutElement&
	...
	const LayoutElement&
	LayoutElement::T() const
	{
		return const_cast<LayoutElement&>(*this).T();
	}
	size_t
	LayoutElement::GetOffsetBegin() const
	{
		return *offset;
	}
	size_t
	LayoutElement::GetOffsetEnd() const
	{
		switch (type)
		{
#define X(el) \
	case el:  \
		return *offset + Map<el>::hlslSize;
			LEAF_ELEMENT_TYPES
#undef X
		case Struct:
			{
		...
		case Array:
			{
				const auto& data = static_cast<ExtraData::Array&>(*pExtraData);
				return *offset +
				       AdvanceToBoundary(data.layoutElement->GetSizeInBytes()) * data.size;
			}
		default:
	...
	size_t
	LayoutElement::GetSizeInBytes() const
	{
	...
	size_t
	LayoutElement::Finalize(size_t offsetIn)
	{
		switch (type)
		{
#define X(el)                                                           \
	case el:                                                            \
		offset = AdvanceIfCrossesBoundary(offsetIn, Map<el>::hlslSize); \
		return *offset + Map<el>::hlslSize;
			LEAF_ELEMENT_TYPES
#undef X
		case Struct:
	...
	std::string
	LayoutElement::GetSignatureForStruct() const
	{
	...
	std::string
	LayoutElement::GetSignatureForArray() const
	{
		using namespace std::string_literals;
		const auto& data = static_cast<ExtraData::Array&>(*pExtraData);
		return "Ar:"s + std::to_string(data.size) + "{"s + data.layoutElement->GetSignature() +
		       "}"s;
	}
	size_t
	LayoutElement::FinalizeForStruct(size_t offsetIn)
	{
	...
	size_t
	LayoutElement::FinalizeForArray(size_t offsetIn)
	{
	...
	bool
	LayoutElement::CrossesBoundary(size_t offset, size_t size) noexcept
	{
	...
	size_t
	LayoutElement::AdvanceIfCrossesBoundary(size_t offset, size_t size) noexcept
	{
		return CrossesBoundary(offset, size) ? AdvanceToBoundary(offset) : offset;
	}
	size_t
	LayoutElement::AdvanceToBoundary(size_t offset) noexcept
	{
		return offset + (16u - offset % 16u) % 16u;
	}
	bool
	LayoutElement::ValidateSymbolName(const std::string& name) noexcept
	{
	...
	size_t
	Layout::GetSizeInBytes() const noexcept
	{
		return pRoot->GetSizeInBytes();
	}
	std::string
	Layout::GetSignature() const
	{
	...
	{}
	LayoutElement&
	RawLayout::operator[](const std::string& key)
	{
		return (*pRoot)[key];
	}
	std::shared_ptr<LayoutElement>
	RawLayout::DeliverRoot() noexcept
	{
	...
	{}
	std::shared_ptr<LayoutElement>
	CookedLayout::RelinquishRoot() const noexcept
	{
		return std::move(pRoot);
	}
	std::shared_ptr<LayoutElement>
	CookedLayout::ShareRoot() const noexcept
	{
		return pRoot;
	}
	const LayoutElement&
	CookedLayout::operator[](const std::string& key) const
	{
		return (*pRoot)[key];
	}

	bool
	ConstElementRef::Exists() const noexcept
	{
	...
	ConstElementRef::Ptr
	ConstElementRef::operator&() const
	{
		return Ptr{ this };
	}
	ConstElementRef::ConstElementRef(
		const LayoutElement* pLayout,
		const char*          pBytes,
		size_t               offset) noexcept : offset(offset), pLayout(pLayout), pBytes(pBytes)
	{}
	ConstElementRef::Ptr::Ptr(const ConstElementRef* ref) noexcept : ref(ref) {}

	ElementRef::operator ConstElementRef() const noexcept { return { pLayout, pBytes, offset }; }
	bool
	ElementRef::Exists() const noexcept
	{
		return pLayout->Exists();
	}
	ElementRef
	...
	Buffer::Buffer(RawLayout&& lay) : Buffer(LayoutCodex::Resolve(std::move(lay))) {}
	...
	{}
	Buffer::Buffer(const Buffer& buf) noexcept : pLayoutRoot(buf.pLayoutRoot), bytes(buf.bytes) {}
	...
	ConstElementRef
	Buffer::operator[](const std::string& key) const
	{
		return const_cast<Buffer&>(*this)[key];
	}
	const char*
	Buffer::GetData() const noexcept
	{
		return bytes.data();
	}
	size_t
	Buffer::GetSizeInBytes() const noexcept
	{
		return bytes.size();
	}
	const LayoutElement&
	Buffer::GetRootLayoutElement() const noexcept
	{
		return *pLayoutRoot;
	}
	void
	Buffer::CopyFrom(const Buffer& other)
	{
		assert(&GetRootLayoutElement() == &other.GetRootLayoutElement());
		std::copy(other.bytes.begin(), other.bytes.end(), bytes.begin());
	}
	std::shared_ptr<LayoutElement>
	Buffer::ShareLayoutRoot() const noexcept
	{
		return pLayoutRoot;
	}
}
