	...
	dtheta(odist(rng)), dchi(odist(rng)), chi(adist(rng)), theta(adist(rng)), phi(adist(rng))
{
	...
	auto pvs   = std::make_unique<VertexShader>(gfx, L"shaders/vs_test.cso");
	auto pvsbc = pvs->GetBytecode();
	AddBind(std::move(pvs));

	AddBind(std::make_unique<PixelShader>(gfx, L"shaders/ps_test.cso"));

	const std::vector<unsigned short> indices = { 0, 2, 1, 2, 3, 1, 1, 3, 5, 3, 7, 5,
